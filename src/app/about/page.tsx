...
import { foodDatabase, detailedRecipes } from '@/lib/data';
...
const IconExplanation: React.FC<{ food: Food; icon: React.ReactNode; name: string; description: string; }> = ({ food, icon, name, description }) => (
...
const NutrientDetail: React.FC<{ label: string; value: number; unit: string; max: number; color: string; }> = ({ label, value, unit, max, color }) => (
    <div>
        <div className="flex justify-between text-sm">
            <span className="font-medium">{label}</span>
            <span className="text-muted-foreground">{value.toFixed(1)}{unit}</span>
        </div>
        <Progress value={(value / max) * 100} className={`h-2 [&>*]:bg-${color}`} />
    </div>
);
...
export default function AboutPage() {
    ...
    const handleIconClick = (food: Food) => {
    ...
    const foodIcons = {
        protein: [
            ...
            { id: 'f32', icon: <Beef className="h-6 w-6" />, desc: 'proteinSource' },
        ],
        carbs: [
    ...
    const relatedRecipes = selectedFood ? detailedRecipes[language].filter(recipe => 
        recipe.ingredients.some(ingredient => ingredient.food.id === selectedFood.id)
    ) : [];

    return (
        <Dialog onOpenChange={(isOpen) => !isOpen && setSelectedFood(null)}>
                ...
                <div className="max-w-4xl mx-auto space-y-12">
                    ...
                    <main className="space-y-10">
                        ...
                        <Accordion type="single" collapsible>
                            <AccordionItem value="food-and-beverage">
                                <AccordionTrigger>
                                    <Section icon={<Utensils className="h-7 w-7" />} title={t('foodAndBeverageTitle')}>
                                    </Section>
                                </AccordionTrigger>
                                <AccordionContent className="pt-4 space-y-6">
                                    <p className="text-muted-foreground -mt-4 px-1">{t('foodAndBeverageContent')}</p>
                                    <IconCategoryCard title={t('proteinSource')}>
                                        {foodIcons.protein.map(item => {
                                            const food = foodDatabase.find(f => f.id === item.id);
                                            if (!food) return null;
                                            return <div key={item.id} onClick={() => handleIconClick(food)}><IconExplanation food={food} icon={item.icon} name={t(item.id as any)} description={t(item.desc as any)} /></div>
                                        })}
                                    </IconCategoryCard>
                                    <IconCategoryCard title={t('carbohydrateSource')}>
                                        {foodIcons.carbs.map(item => {
                                            const food = foodDatabase.find(f => f.id === item.id);
                                            if (!food) return null;
                                            return <div key={item.id} onClick={() => handleIconClick(food)}><IconExplanation food={food} icon={item.icon} name={t(item.id as any)} description={t(item.desc as any)} /></div>
                                        })}
                                    ...
                                    <IconCategoryCard title={t('fruitAndVegetableSource')}>
                                        {foodIcons.fruitsAndVeg.map(item => {
                                            const food = foodDatabase.find(f => f.id === item.id);
                                            if (!food) return null;
                                            return <div key={item.id} onClick={() => handleIconClick(food)}><IconExplanation food={food} icon={item.icon} name={t(item.id as any)} description={t(item.desc as any)} /></div>
                                        })}
                                    ...
                                    <IconCategoryCard title={t('snacksAndDrinks')}>
                                        {foodIcons.snacksAndDrinks.map(item => {
                                            const food = foodDatabase.find(f => f.id === item.id);
                                            if (!food) return null;
                                            return <div key={item.id} onClick={() => handleIconClick(food)}><IconExplanation food={food} icon={item.icon} name={t(item.id as any)} description={t(item.desc as any)} /></div>
                                        })}
            ...
            <DialogContent className="sm:max-w-md">
                {selectedFood && (
                    <>
                        <DialogHeader>
                            <DialogTitle className="text-2xl">{t(selectedFood.id as any)}</DialogTitle>
                            <CardDescription>{t('nutrientDetails')} ({selectedFood.servingSize})</CardDescription>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
                             <div className="text-center">
                                <p className="text-4xl font-bold font-headline text-primary">{selectedFood.calories}</p>
                                <p className="text-sm text-muted-foreground">{t('calories')}</p>
                            </div>
                            <Separator />
                            <div className="space-y-3">
                                <h4 className="font-semibold">{t('macronutrients')}</h4>
                                <NutrientDetail label={t('protein')} value={selectedFood.protein} unit="g" max={50} color="chart-1" />
                                <NutrientDetail label={t('carbohydrates')} value={selectedFood.carbs} unit="g" max={50} color="chart-2" />
                                <NutrientDetail label={t('fat')} value={selectedFood.fat} unit="g" max={20} color="chart-3" />
                            </div>
                             <Separator />
                            <div className="space-y-3">
                                <h4 className="font-semibold">{t('micronutrients')}</h4>
                                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                    {selectedFood.nutrients.vitaminC > 0 && <p>Vitamin C: {selectedFood.nutrients.vitaminC}mg</p>}
                                    {selectedFood.nutrients.iron > 0 && <p>{t('iron')}: {selectedFood.nutrients.iron}mg</p>}
                                    {selectedFood.nutrients.calcium > 0 && <p>{t('calcium')}: {selectedFood.nutrients.calcium}mg</p>}
                                    {selectedFood.nutrients.fiber > 0 && <p>{t('fiber')}: {selectedFood.nutrients.fiber}g</p>}
                                </div>
                            </div>
                            {relatedRecipes.length > 0 && (
                                ...
                                <Separator />
                                <div className="space-y-3">
                                    <h4 className="font-semibold">{t('relatedRecipes')}</h4>
                                    <ul className="list-disc list-inside text-sm text-primary">
                                        {relatedRecipes.map(recipe => (
                                            <li key={recipe.name} className="hover:underline cursor-pointer">{recipe.name}</li>
                                        ))}
                                    </ul>
                                </div>
                                </>
